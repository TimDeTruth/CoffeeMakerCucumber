...
public class TestSteps {
	...
	@When("^user edits recipe -?(\\d+) which doesn't exist$")
	...
	public void user_add_inventory(DataTable addInventory) throws Throwable {
		...
		Command cmd4 = new AddInventory(add.get(0).get("coffee"), add.get(0).get("milk"), add.get(0).get("sugar"),
				add.get(0).get("chocolate"));
	...
	public void check_updated_recipe(int recipeIndex) throws Throwable {
		// Write code here that turns the phrase above into concrete actions
		assertEquals("Coffee", recipeBook.getRecipes()[recipeIndex].getName());

		assertEquals(2, recipeBook.getRecipes()[recipeIndex].getAmtCoffee());
		assertEquals(12, recipeBook.getRecipes()[recipeIndex].getPrice());
		assertEquals(5, recipeBook.getRecipes()[recipeIndex].getAmtMilk());
		assertEquals(4, recipeBook.getRecipes()[recipeIndex].getAmtSugar());
		assertEquals(3, recipeBook.getRecipes()[recipeIndex].getAmtChocolate());
	}
	...
	public void user_delete_Recipe(int recipeToDelete) throws Throwable {
		// Write code here that turns the phrase above into concrete actions
		Command cmd = new ChooseService(2);
		coffeeMakerMain.UI_Input(cmd);
		Command cmd2 = new ChooseRecipe(recipeToDelete);
		coffeeMakerMain.UI_Input(cmd2);
	}
	...
	@When("^insert money (\\d+)$")
	public void insert_money(int moneyAmount) throws Throwable {
		// Write code here that turns the phrase above into concrete actions
		Command cmd5 = new InsertMoney(moneyAmount);
		coffeeMakerMain.UI_Input(cmd5);
	}

	@When("^purchase coffee (\\d+)$")
	public void purchase_coffee(int drinkIndex) throws Throwable {
		// Write code here that turns the phrase above into concrete actions
		coffeeMakerMain.UI_Input(new ChooseService(6));
		coffeeMakerMain.UI_Input(new ChooseRecipe(drinkIndex));
	}

	@Then("^we should take money from tray (\\d+)$")
	public void we_should_take_money_from_tray(int moneyAmount) throws Throwable {
		// Write code here that turns the phrase above into concrete actions
		assertEquals(moneyAmount, coffeeMakerMain.moneyInTray);
	}
